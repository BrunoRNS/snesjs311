...
// Função para carregar o ROM a partir de um ArrayBuffer
function loadRomFromArrayBuffer(buffer) {
  let romArr = new Uint8Array(buffer);
  loadRom(romArr); // Supondo que loadRom seja uma função definida em outro lugar
}

// Carrega o arquivo main.smc diretamente
fetch('./main.smc').then(response => {
  if (response.ok) {
    response.arrayBuffer().then(buffer => {
      loadRomFromArrayBuffer(buffer);
    });
  } else {
    console.error('Não foi possível carregar o arquivo main.smc');
  }
}).catch(error => {
  console.error('Erro ao buscar o arquivo main.smc:', error);
});
