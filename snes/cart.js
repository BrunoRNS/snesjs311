...
class Cart {
  constructor(data, header, isHirom) {
    this.header = header;
    this.data = data;
      ...
      if (!this.isHirom) {
        if (adr < 0x8000) {
          if (bank >= 0x70 && bank < 0x7e && this.hasSram) {
            // sram
            return this.sram[(((bank - 0x70) << 15) | (adr & 0x7fff)) & (this.sramSize - 1)];
          }
        }
        ...
        if (adr >= 0x6000 && adr < 0x8000 && this.hasSram) {
          if ((bank < 0x40 || (bank >= 0x80 && bank < 0xc0))) {
            // sram
            return this.sram[(((bank & 0x3f) << 13) | (adr & 0x1fff)) & (this.sramSize - 1)];
          }
        }
    ...
    this.write = function (bank, adr, value) {
      if (!this.isHirom) {
        if (adr < 0x8000 && bank >= 0x70 && bank < 0x7e && this.hasSram) {
          this.sram[(((bank - 0x70) << 15) | (adr & 0x7fff)) & (this.sramSize - 1)] = value;
        }
      } else {
        if (adr >= 0x6000 && adr < 0x8000 && this.hasSram) {
          if ((bank < 0x40 || (bank >= 0x80 && bank < 0xc0))) {
            // sram
            this.sram[(((bank & 0x3f) << 13) | (adr & 0x1fff)) & (this.sramSize - 1)] = value;
          }
        }
      }
    };
